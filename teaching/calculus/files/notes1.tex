...
\end{definition}
That is, the existence of a limit depends only on the behavior of the function near the point $a$, regardless of whether $f(a)$ is defined. In particular, the limit $\lim_{x\to a} f(x)$ exists if and only if any sequence of numbers $\{x_n\}$ approaching $a$ produces a sequence of 
numbers $\{f(x_n)\}$ approaching the same value $L$. Since we can approach $a$ from both sides (either right or left) on the real number line, for the overall limit to exist, 
