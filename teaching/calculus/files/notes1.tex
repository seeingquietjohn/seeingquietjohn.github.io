...
\begin{example}
    Consider the limit $\lim_{x\to0} x^2 \sin\frac{1}{x}$. Notice that we cannot algebraically compute this limit since \[\lim_{x\to0}\sin\frac{1}{x}\] 
...
\end{align*}
Since $\lim_{x\to0} -x^2 = \lim_{x\to0} x^2 = 0$, we conclude that $\lim_{x\to0} x^2 \sin\frac{1}{x} = 0$ as well even though the limit of $\sin\frac{1}{x}$ does not exist.  
\end{example}
